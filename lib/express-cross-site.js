    ...
    this.create = function(userSettings) {
        ...
        return new function() {
            ...
            this.token = function(req, res) {
                ...
                var pool = req.session[settings['storage']] = clean(req.session[settings['storage']]);
            ...
            this.check = function(userSettings) {
                ...
                return function(req, res, next) {
                    ...
                    if (enabled(req, res)) {
                        ...
                        error_callback(new CSRFAttackError(csrf+' token not present in the pool ' + JSON.stringify(pool, 2)), req, res, next);
                        return;
                    } 
                    success_callback(req, res, next);
                };
