    ...
    this.create = function(userSettings) {
        ...
        return new function() {
            ...
            this.check = function(userSettings) {
                var chekSettings = extend(defaultCheckSettings, userSettings);
                
                return function(req, res, next) {
                    if (chekSettings['enabled'] && chekSettings.methods.indexOf(req.method.toLowerCase()) != -1) {
                        var body = req.body || {};
                        var csrf = body[chekSettings['param']];
                        var error_callback = chekSettings['error_callback'];
                        var success_callback = chekSettings['success_callback'];
